...
from collector.passes.channel_template_pass import EnhancedChannelTemplatePass, ChannelPattern
...
class TestMultiPassController:
    ...
    async def test_pass_progression_on_low_confidence(self):
        ...
        with patch.object(controller, '_pass_0_channel_template') as mock_pass_0, \
             patch.object(controller, '_pass_1_auto_retemplate') as mock_pass_1:
    ...
    async def test_budget_exceeded_stops_processing(self):
        ...
        config = MultiPassConfig(
            enabled=True,
            total_cpu_budget=5.0,  # Very low budget
            total_api_budget=10
        )
        ...
        with patch.object(controller, '_pass_0_channel_template', slow_pass), \
             patch.object(controller, '_pass_1_auto_retemplate', slow_pass), \
...
class TestChannelTemplatePass:
    ...
    def setup_method(self):
        ...
        self.pass_instance = EnhancedChannelTemplatePass(
            self.mock_advanced_parser,
            None  # No db_manager for tests
        )
        ...
        test_cases = [
            {
                ...
                "expected": "ARTIST-SONG-KARAOKE"
            },
            {
                "title": '"Artist" - "Song" Karaoke',
                "expected": "QUOTED-ARTIST-SONG-KARAOKE"
            },
            {
                "title": 'Song Title by Artist Name (Karaoke)',
                "expected": "SONG-BY-ARTIST-KARAOKE"
            }
        ]
...
class TestAcousticFingerprintPass:
    ...
    def setup_method(self):
        """Set up test fixtures."""
        self.mock_advanced_parser = Mock(spec=AdvancedTitleParser)
        self.pass_instance = AcousticFingerprintPass(
            self.mock_advanced_parser,
            None
        )
...
class TestIntegrationScenarios:
    ...
    async def test_fallback_through_multiple_passes(self):
        ...
        with patch.object(self.controller.channel_template_pass, 'parse', return_value=None), \
             patch.object(self.controller.auto_retemplate_pass, 'parse', return_value=None), \
             patch.object(self.controller.ml_embedding_pass, 'parse', return_value=None), \
             patch.object(self.controller.web_search_pass, 'parse') as mock_web_search:
    ...
    async def test_complete_failure_scenario(self):
        ...
        with patch.object(self.controller.channel_template_pass, 'parse', return_value=None), \
             patch.object(self.controller.auto_retemplate_pass, 'parse', return_value=None), \
             patch.object(self.controller.ml_embedding_pass, 'parse', return_value=None), \
             patch.object(self.controller.web_search_pass, 'parse', return_value=None), \
             patch.object(self.controller.acoustic_fingerprint_pass, 'parse', return_value=None):
    ...
    async def test_budget_management_integration(self):
        ...
        with patch.object(controller, '_pass_0_channel_template', budget_consuming_pass), \
