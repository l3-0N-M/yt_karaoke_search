            ...
            while retry_count <= max_retries:
                try:
                    ...
                    if (
                        HAS_AIOHTTP
                        and aiohttp
                        and hasattr(aiohttp, "ClientResponseError")
...
class DiscogsClient:
    ...
    async def search_release(
        self, artist: str, track: str, max_results: int = 10, timeout: int = 10
    ) -> List[DiscogsMatch]:
        ...
        for artist_var in artist_variations:
            search_queries.append(
                {"artist": artist_var, "track": normalized_track, "type": "release"}
            )
        ...
        if "(" in normalized_track and ")" in normalized_track:
            ...
            if track_without_parens and track_without_parens != normalized_track:
                for artist_var in artist_variations[:2]:  # Only use top 2 artist variations
                    search_queries.append(
                        {"artist": artist_var, "track": track_without_parens, "type": "release"}
                    )
        ...
        if len(search_queries) < 6:  # Limit total queries
            search_queries.append({"query": f"{artist} {normalized_track}", "type": "release"})
            ...
            try:
                ...
                async with aiohttp.ClientSession(
                    ...
                    async with session.get(
                        f"{self.BASE_URL}/database/search", params=query_params
                    ) as response:
                ...
                self.logger.warning(
                    f"Discogs search timeout for {query_params.get('artist')} - {query_params.get('track')}"
                )
                ...
                if (
                    ...
                    if e.status == 429:
                        ...
                        self.rate_limiter.handle_429_error(retry_after)
                        self.logger.warning(
                            f"Discogs API rate limit hit (429) for {query_params}. Retry after: {retry_after}s"
                        )
                        ...
                        self.logger.warning(
                            f"Discogs API HTTP error {e.status} for query {query_params}: {e}"
                        )
                        continue  # Try next query
                elif (
                    HAS_AIOHTTP
                    and aiohttp
                    and hasattr(aiohttp, "ClientError")
                    and isinstance(e, aiohttp.ClientError)
                ):
                ...
                if self.monitor:
                    self.monitor.record_api_call(
                        success=success, response_time_ms=response_time, timeout=timeout_occurred
                    )
    ...
    def _parse_search_result(
        self, item: Dict, query_artist: str, query_track: str
    ) -> Optional[DiscogsMatch]:
        ...
        try:
            ...
            artist_similarity = self._text_similarity(
                query_artist.lower(), artist_name.lower(), is_artist=True
            )
            ...
            if item.get("format"):
                format_info = (
                    item["format"][0] if isinstance(item["format"], list) else item["format"]
                )
    ...
    def _text_similarity(self, text1: str, text2: str, is_artist: bool = False) -> float:
        ...
        if len(text1_clean) >= 3 and len(text2_clean) >= 3:
            if text1_clean in text2_clean or text2_clean in text1_clean:
                substring_similarity = min(len(text1_clean), len(text2_clean)) / max(
                    len(text1_clean), len(text2_clean)
                )
        ...
        if is_artist:
            ...
            for q_word in query_words:
                ...
                for r_word in result_words:
                    word_sim = 1.0 - (
                        levenshtein_distance(q_word, r_word) / max(len(q_word), len(r_word))
                    )
            ...
            combined_similarity = (
                edit_similarity * 0.6 + token_similarity * 0.25 + substring_similarity * 0.15
            )
        ...
        try:
            ...
            if not best_match or best_confidence < self.config.discogs_confidence_threshold:
                ...
                self.monitor.record_search_attempt(
                    success=False, confidence=best_confidence, fallback=is_fallback
                )
                return None
            ...
            discogs_metadata = {
                ...
                "genre": (
                    best_match.genres[0] if best_match.genres else None
                ),  # Use first genre as string
            ...
            logger.info(
                f"Discogs metadata for {best_match.artist_name} - {best_match.song_title}: "
            ...
            self.monitor.record_search_attempt(
                success=True, confidence=best_confidence, fallback=is_fallback
            )
