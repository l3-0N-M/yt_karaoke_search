...
class EnhancedChannelTemplatePass:
    ...
    def parse(
        ...
        try:
            ...
            fallback_result = self.advanced_parser._parse_with_channel_patterns(
                title, channel_name
            )
    ...
    def _try_channel_patterns(
        ...
        patterns.sort(key=lambda p: (
            p.success_count / max(p.total_attempts, 1),
            p.last_used
        ), reverse=True)
    ...
    def _enhanced_channel_detection(
        ...
        enhanced_patterns = [
            ...
            (
                fr"^{re.escape(channel_name)}[:\s]*[\"']([^\"']+)[\"']\s*[-–—]\s*[\"']([^\"']+)[\"']",
                1, 2, 0.9, "channel_branded_quoted"
            ),
            (
                fr"^\[{re.escape(channel_name)}\]\s*([^-–—]+)[-–—](.+)",
                1, 2, 0.85, "channel_bracketed"
            ),
            (
                fr"^{re.escape(channel_name)}\s*[-–—]\s*(.+?)\s*[-–—]\s*(.+?)\s*\([^)]*[Kk]araoke[^)]*\)",
                1, 2, 0.8, "channel_double_dash"
            ),
            
            # Popular karaoke channel specific patterns
            (
                r'^"([^"]+)"\s*[-–—]\s*"([^"]+)"\s*[-–—]\s*[Kk]araoke',
                1, 2, 0.9, "double_quoted_karaoke"
            ),
            (
                r'^([^-–—]+)\s*[-–—]\s*"([^"]+)"\s*\([^)]*[Kk]araoke[^)]*\)',
                1, 2, 0.85, "artist_quoted_title"
            ),
            (
                r'^\[([^\]]+)\]\s*([^-–—]+)[-–—](.+?)\s*\[[^\]]*[Kk]araoke[^\]]*\]',
                2, 3, 0.8, "bracketed_artist_title"
            ),
        ]
            ...
            try:
                ...
                if match:
                    result = self.advanced_parser._create_result_from_match(
                        ...
                        f"enhanced_channel_{pattern_name}", pattern
                    )
                    if result and result.confidence > 0:
                        return result
    ...
    def _extract_with_channel_context(
        ...
        karaoke_indicators = [
            'karaoke', 'karaoké', 'karaokê', 'караоке',
            'sing along', 'backing track', 'instrumental',
            'covers', 'tribute', 'piano version'
        ]
        ...
        for pattern, artist_group, title_group, base_confidence, pattern_name in simple_patterns:
            try:
                ...
                if match:
                    result = self.advanced_parser._create_result_from_match(
                        match, artist_group, title_group, base_confidence,
                        f"channel_context_{pattern_name}", pattern
                    )
                    if result and result.confidence > 0:
    ...
    def _generalize_pattern(self, title: str, result: ParseResult) -> Optional[str]:
        ...
        artist_escaped = re.escape(result.original_artist)
        title_escaped = re.escape(result.song_title)
    ...
    def _cleanup_patterns(self, channel_id: str):
        ...
        for pattern in patterns:
            ...
            if (age_days < self.pattern_decay_days or 
                success_rate > 0.6 or 
                (now - pattern.last_used).days < 7):
    ...
    def _refresh_channel_patterns(self, channel_id: str):
        ...
        if channel_id in self.channel_patterns:
            ...
            self.channel_patterns[channel_id] = [
                p for p in patterns 
                if (p.success_count / max(p.total_attempts, 1)) > 0.3
            ]
