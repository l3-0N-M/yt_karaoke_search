...
class AutoRetemplatePass:
    ...
    def _try_active_patterns(
        ...
        sorted_patterns = sorted(
            ...
            key=lambda p: (
                p.last_seen,
                p.success_count / max(p.video_count, 1)
            ),
            reverse=True
        )
    ...
    def _attempt_learning(
        ...
        basic_result = self.advanced_parser.parse_title(title, description, tags, trend.channel_name)
    ...
    def _should_analyze_patterns(self, trend: ChannelTrend) -> bool:
        ...
        if (len(trend.active_patterns) < 2 or 
            trend.successful_recent_parses / max(trend.total_recent_videos, 1) < 0.6):
            return True
    ...
    def get_statistics(self) -> Dict:
        ...
        channels_with_changes = sum(
            1 for trend in self.channel_trends.values() 
            if trend.pattern_change_detected
        )
        ...
        return {
            ...
            "avg_patterns_per_channel": (
                total_active_patterns / max(total_channels, 1)
            ),
            "pattern_change_rate": (
                channels_with_changes / max(total_channels, 1)
            )
        }