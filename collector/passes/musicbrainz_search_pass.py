...
class MusicBrainzSearchPass:
    ...
    def _generate_search_queries(self, title: str) -> List[str]:
        ...
        for separator in [" - ", " by ", " from ", ":", " | "]:
            if separator in base_query:
                ...
                if len(parts) == 2:
                    # Try both orders
                    queries.append(f'artist:"{parts[0].strip()}" AND recording:"{parts[1].strip()}"')
                    queries.append(f'artist:"{parts[1].strip()}" AND recording:"{parts[0].strip()}"')
                break
        ...
        minimal = re.sub(r"\b(?:karaoke|instrumental|backing|track)\b", "", title, flags=re.IGNORECASE).strip()
    ...
    def _calculate_confidence(self, mb_score: int, title: str, artist: str, query: str) -> float:
        ...
        combined_similarity = SequenceMatcher(None, f"{artist_lower} {title_lower}", query_lower).ratio()
