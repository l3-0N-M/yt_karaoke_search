...
async def demonstrate_multi_pass_parsing():
    ...
    for i, test_case in enumerate(test_titles, 1):
        ...
        if result.final_result:
            ...
            print(f"  Stopped at: {result.stopped_at_pass.value if result.stopped_at_pass else 'None'}")
        else:
    ...
    print(f"   Success rate: {successful_parses}/{len(test_titles)} ({successful_parses/len(test_titles)*100:.1f}%)")
    print(f"   Average processing time: {total_processing_time/len(test_titles):.2f}s")
    print(f"   Average CPU per video: {stats.get('budget_efficiency', {}).get('avg_cpu_per_video', 0):.2f}s")
    print(f"   Average API per video: {stats.get('budget_efficiency', {}).get('avg_api_per_video', 0):.1f} calls")
    ...
    if hasattr(controller, 'channel_template_pass'):
        ...
        print(f"   Pass 0 (Channel Template):")
    ...
    if hasattr(controller, 'auto_retemplate_pass'):
        ...
        print(f"   Pass 1 (Auto-retemplate):")
    ...
    if hasattr(controller, 'web_search_pass') and controller.web_search_pass:
        ...
        print(f"   Pass 3 (Web Search):")
        print(f"     Cache hit rate: {search_stats.get('cache_hit_rate', 0):.1%}")
        print(f"     Cache entries: {search_stats.get('cache_statistics', {}).get('total_entries', 0)}")
        print(f"     Search success rate: {search_stats.get('success_rate', 0):.1%}")
    else:
        print(f"   Pass 3 (Web Search): Not enabled (requires search engine)")
    ...
    if hasattr(controller, 'acoustic_fingerprint_pass'):
        ...
        print(f"   Pass 4 (Acoustic Fingerprint):")
        ...
        if not acoustic_stats.get('enabled', False):
            print(f"     Status: Placeholder implementation")
