...
class VideoProcessor:
    ...
    async def _enhance_with_external_data(self, video_data: Dict, errors: List[str], warnings: List[str]):
        ...
        if (self.config.data_sources.musicbrainz_enabled and 
            features.get('original_artist') and features.get('song_title')):
    ...
    async def _get_music_metadata(self, artist: str, song: str) -> Dict:
        ...
        try:
            ...
            response = await self.http_client.get(
                url, 
                headers=headers, 
                timeout=self.config.data_sources.musicbrainz_timeout
            )
                ...
                if recordings:
                    ...
                    if releases:
                        ...
                        if release_date and len(release_date) >= 4:
                            ...
                            return {
                                'estimated_release_year': year,
                                'release_year_confidence': 0.8
                            }
        except Exception as e:
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                if len(part2) < len(part1) and len(part2) < 50:
                    return {
                        'song_title': part1,
                        'original_artist': part2,
                        'artist_confidence': 0.8
                    }
                else:
                    return {
                        'song_title': part2, 
                        'original_artist': part1,
                        'artist_confidence': 0.7
                    }
        
        desc_match = re.search(r'(?:by|artist):?\s*(.+?)(?:\n|$)', description, re.IGNORECASE)
        if desc_match:
            return {
                'original_artist': desc_match.group(1).strip(),
                'artist_confidence': 0.6
            }
    ...
    def _calculate_confidence_score(self, video_data: Dict, errors: List[str], warnings: List[str]) -> float:
        ...
        confidence_keys = [
            ...
            'is_piano_only_confidence', 'is_acoustic_confidence'
        ]
        valid_confidences = [features.get(key, 0) for key in confidence_keys if features.get(key, 0) > 0]
        feature_confidence = sum(valid_confidences) / len(valid_confidences) if valid_confidences else 0
