...
class MultiPassParsingController:
    ...
    def __init__(
        ...
        self.channel_template_pass = EnhancedChannelTemplatePass(
            advanced_parser, db_manager
        )
        self.auto_retemplate_pass = AutoRetemplatePass(
            advanced_parser, db_manager
        )
        ...
        self.ml_embedding_pass = EnhancedMLEmbeddingPass(
            advanced_parser, fuzzy_matcher, db_manager
        )
        
        if search_engine:
            self.web_search_pass = EnhancedWebSearchPass(
                advanced_parser, search_engine, db_manager
            )
        else:
            self.web_search_pass = None
            
        self.acoustic_fingerprint_pass = AcousticFingerprintPass(
            advanced_parser, db_manager
        )
    ...
    async def _execute_parsing_ladder(
        ...
        for pass_type in PassType:
            ...
            pass_result = await self._execute_single_pass(
                pass_type, pass_config, title, description, tags, 
            ...
            if (pass_result.success and 
                pass_result.confidence >= pass_config.confidence_threshold):
    ...
    def _check_budget_exceeded(self, result: MultiPassResult, pass_config: PassConfig) -> bool:
        """Check if executing this pass would exceed budget limits."""
        
        # Check CPU budget
        if (result.budget_consumed["cpu"] + pass_config.cpu_budget_limit > 
            self.config.total_cpu_budget):
            return True
            
        # Check API budget  
        if (result.budget_consumed["api"] + pass_config.api_budget_limit > 
    ...
    def get_statistics(self) -> Dict:
        ...
        if total_videos > 0:
            stats["budget_efficiency"] = {
                "avg_cpu_per_video": self.statistics["budget_consumption"]["cpu_total"] / total_videos,
                "avg_api_per_video": self.statistics["budget_consumption"]["api_total"] / total_videos
            }
    ...
    async def _pass_0_channel_template(
        ...
        channel_name: str, channel_id: str, metadata: Dict
