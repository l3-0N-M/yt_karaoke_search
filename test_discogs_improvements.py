...
from collector.config import CollectorConfig, DataSourceConfig
...
async def test_discogs_search():
    ...
    client = DiscogsClient(
        ...
        user_agent=config.data_sources.discogs_user_agent
    )
        ...
        try:
            ...
            if matches:
                ...
                if best_match.confidence >= 0.4:  # New threshold
                    successful += 1
                    print(f"✅ Found: {best_match.artist_name} - {best_match.song_title} "
                          f"(confidence: {best_match.confidence:.2f}, year: {best_match.year})")
                else:
                    print(f"⚠️  Low confidence: {best_match.confidence:.2f} for "
