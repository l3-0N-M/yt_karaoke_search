...
def test_advanced_parser():
    ...
    if config.search.use_advanced_parser:
        ...
        print(f"Advanced parser result:")
        print(f"  original_artist: '{result.original_artist}' (length: {len(result.original_artist) if result.original_artist else 'None'})")
        print(f"  song_title: '{result.song_title}' (length: {len(result.song_title) if result.song_title else 'None'})")
        ...
        print(f"  Core patterns result:")
        print(f"    original_artist: '{core_result.original_artist}' (length: {len(core_result.original_artist) if core_result.original_artist else 'None'})")
        print(f"    song_title: '{core_result.song_title}' (length: {len(core_result.song_title) if core_result.song_title else 'None'})")
        ...
        for i, (pattern, artist_group, title_group, confidence, pattern_name) in enumerate(parser.core_patterns):
            import re
