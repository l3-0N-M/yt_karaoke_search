            ...
            for col in columns:
                ...
                if null_count > 0 and col[3]:  # NOT NULL constraint violated
                    quality_issues.append(
                        f"Column {col_name} has {null_count} nulls despite NOT NULL constraint"
                    )
                ...
                if "TEXT" in col_type.upper():
                    ...
                    if empty_count > 0:
                        ...
                        print(
                            f"  - {col_name}: {null_count} nulls ({null_percentage:.1f}%), {empty_count} empty strings ({empty_percentage:.1f}%)"
                        )
                else:
                    if null_count > 0:
    ...
    if "videos" in tables:
        ...
        for fk in foreign_keys:
            ...
            if cursor.fetchone()[0] == 0:
                ...
                analysis["performance"]["missing_indexes"] = analysis["performance"].get(
                    "missing_indexes", []
                )
